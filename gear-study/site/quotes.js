...
window.QUOTE_DATA = (() => {
  ...
  // ---------- crew chief hints (asked as questions, never the answer) ----------
  const HINT = {
    mains:"Where's the PA? You need main speakers.", wedge:"A band needs mains and monitor wedges.",
    linearray:"That crowd is big and outside. Party speakers won't throw that far. Line array?",
    sub:"Are you forgetting a sub?", console:"What's mixing all this? Add a console.",
    hh:"They asked for handheld wireless.", pack:"Hands-free mics for the presenters? That's bodypacks.",
    lav:"Those bodypacks need a lav or headset plugged in.", rxch:"Every wireless mic needs its own receiver channel.",
    lectern:"Mic on the podium?", podium:"Podium?", tv55:"TVs? Check the size they asked for.", tv65:"TVs? Check the size they asked for.",
    tv70:"TVs? Check the size they asked for.", tvstand:"What are the TVs standing on?", projector:"Projector?",
    screen:"What are you projecting onto?", uplight:"Uplights: count what they asked for.", lightctl:"Who's running the lights? Add a controller.",
    dmx:"The lights need DMX cable.", camera:"Cameras for the stream?", switcher:"Something has to cut between the cameras.",
    sdi:"The cameras need SDI runs to the switcher.", intercom:"How does the crew talk to each other?",
    stereodi:"How does the laptop get into the board? Stereo DI.", di:"Bass and keys need DI boxes.", vocal:"Mics for the singers?",
    instmic:"Drums and amps need mics.", micstand:"What are the mics standing on?", spkstand:"Speakers on stands?",
    xlr:"You need cables.", gen:"Is there power out there? Generator.",
  };
  const LHINT = {a1:"Who's mixing? Book an A1.", av:"Who's running video and the laptop? Book an A/V tech.", ld:"Who's running the lights? Book a lighting op.",
    vo:"Who's cutting cameras? Book a video operator.", hand:"Who's loading in? Book stagehands."};

  // ---------- the calls ----------
  // need: [key, n, why]; labor: [role, people, hours per day]; ref: the shop's own quote [id, qty]
  const CALLS = [
    {id:1, budget:3500, who:"Dana, HR events", org:"a pharma company", town:"New Brunswick", room:"Hotel ballroom", ppl:400, days:1,
    ...
    {id:2, budget:2000, who:"Maria, the bride", org:"a wedding", town:"Asbury Park", room:"Reception hall", ppl:150, days:1,
    ...
    {id:3, budget:3250, who:"Mr. Alvarez, vice principal", org:"a high school", town:"Toms River", room:"Football field, outdoors", ppl:800, days:1,
    ...
    {id:4, budget:3750, who:"Priya, comms director", org:"a university think tank", town:"Princeton", room:"Lecture hall", ppl:120, days:1,
    ...
    {id:5, budget:1800, who:"Sal, owner", org:"a brewery", town:"Jersey City", room:"Taproom", ppl:180, days:1,
    ...
    {id:6, budget:8000, who:"Jordan, marketing", org:"a sneaker brand", town:"Hoboken", room:"Loft event space", ppl:250, days:2,
    ...
    {id:7, budget:15000, who:"Pastor Reggie", org:"a church conference", town:"Atlantic City", room:"Convention hall", ppl:600, days:3,
    ...
    {id:8, budget:4500, who:"Evelyn, gala chair", org:"a hospital foundation", town:"Morristown", room:"Estate ballroom", ppl:300, days:1,
    ...
    {id:9, budget:2000, who:"Kevin, trade show manager", org:"a flooring supplier", town:"Edison", room:"Convention center booth", ppl:0, days:2,
    ...
    {id:10, budget:1750, who:"Chris, parks & rec", org:"the town", town:"Montclair", room:"Park lawn, outdoors", ppl:200, days:1,
  ...
  function hints(call, lines, labor, delivery, total){
    const out = [];
    for(const [k,n] of call.need){ const have = count(k, lines); if(have < n) out.push({t: HINT[k], c: `${needLabel(k)}: ${have} of ${n}`}); }
    for(const [k,n,h] of call.labor){ const m = labor.find(l => l[0] === k); if(!m || m[1] < n) out.push({t: LHINT[k], c: `${LAB[k].l}: ${m?m[1]:0} of ${n}`}); }
    if(!delivery) out.push({t: "How's it getting there? Add delivery.", c: "Truck"});
    if(total > call.budget) out.push({t: `You're ${usd(total - call.budget)} over budget. Could cheaper gear or fewer crew hours do the job?`, c: "Budget", over: true});
    return out;
  }
  function grade(call, lines, labor, delivery, myRates){
    ...
    const r = mine.total / ref.total, over = mine.total - call.budget;
    const verdict = over > 0 ? ["Over budget", `${usd(over)} over the client's ${usd(call.budget)}. They'll shop it around.`]
      : r < .65 ? ["Money left on the table", "Inside budget, but far under the shop's number. Something is missing or underpriced."]
      : r < .8 ? ["Tight margin", "Inside budget and under the shop's number. Check you didn't forget anything."]
      : ["On budget", `Inside the client's ${usd(call.budget)} and close to the shop's own number.`];
    checks.push({ok: over <= 0, label: "Within the client's budget", detail: over > 0 ? `${usd(mine.total)} is ${usd(over)} over ${usd(call.budget)}.` : `${usd(mine.total)} of ${usd(call.budget)}. ${usd(-over)} to spare.`});
    checks.push({ok: r >= .8, label: "Priced to make money", detail: r >= .8 ? `Yours ${usd(mine.total)}, shop ${usd(ref.total)}.` : `Yours ${usd(mine.total)} vs the shop's ${usd(ref.total)}. Too low to be real.`});
  ...
  return {
    CALLS, LABOR, DELIVERY, MIN_HRS, dayFactor, rate, grade, hints, totals, refTotals, usd,
