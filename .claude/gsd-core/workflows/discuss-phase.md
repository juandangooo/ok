...
<required_reading>
@/home/user/ok/.claude/gsd-core/references/domain-probes.md
@/home/user/ok/.claude/gsd-core/references/gate-prompts.md
@/home/user/ok/.claude/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "/home/user/ok/.claude/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@/home/user/ok/.claude/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```
Apply each entry in `activeHooks` per @/home/user/ok/.claude/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
```
Apply each entry in `activeHooks` per @/home/user/ok/.claude/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
