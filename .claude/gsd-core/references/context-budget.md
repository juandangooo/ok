...
The keys live in `.claude/settings.json` (project) or `/home/user/ok/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/home/user/ok/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
