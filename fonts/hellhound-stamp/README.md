...
|---|---|---|
| `Hellhound Stamp` | `HellhoundStamp.*` | The solid cut. Start here. |
| `Hellhound Stamp Inline` | `HellhoundStampInline.*` | Solid, with a keyline channel cut just inside the edge. |
| `Hellhound Stamp Outline` | `HellhoundStampOutline.*` | Hollow — the perimeter only. |
| `Hellhound Stamp Shadow` | `HellhoundStampShadow.*` | The extrude. Always sits *behind* another cut. |
...
Pick `Hellhound Stamp` from the font menu and type. Lowercase keys produce
...
2. Put the **lower** copy in `Hellhound Stamp Shadow` and colour it (black, or a
   flat spot colour).
3. Put the **upper** copy in `Hellhound Stamp Outline` (or `Inline`, or
...
@font-face {
  ...
  src: url("HellhoundStamp.woff2") format("woff2");
  font-weight: 800;
  font-display: swap;
}
